...
def get_monthly_billing_breakdown(
    ...
    if service_rate.billing_basis == "Flat Rate":
        base_results = calculate_flat_rate(
            contract,
            service_rate,
            settings,
            proration_factor,
        ...
        base_results = calculate_metered_rate(contract,
                                              service_rate,
                                              settings,
                                              actual_billing_start,
                                              end_date,
                                              proration_factor,
                                              total_days)

    else:
        frappe.throw(_("Invalid Billing Basis for contract {0}")
                     .format(contract_name))

    additional_fee = calculate_adittion_fee(service_rate)

    base_results["discountable_amount"] += (
        additional_fee["discountable_amount"]
    )
    base_results["non_discountable_amount"] += (
        additional_fee["non_discountable_amount"]
    )
...
def get_active_service_rate(
    ...
    if not rate_name:
        frappe.throw(_(
            "No active Service Rate found "
            "for category '{0}' on {1}"
            )
            .format(contract.service_category, reference_date))
...
def calculate_flat_rate(
    ...
    cistern_fee = (
        (flt(service_rate.cistern_fee or 0) * factor)
        if contract.has_cistern else 0
    )
    detailed_items = []
    ...
    if settings.separate_cistern_fee:
        ...
        if cistern_fee > 0:
            detailed_items.append({"description": _("Cistern Fee") + suffix,
                                   "amount": cistern_fee})
    else:
        detailed_items.append({
            "description": ("{0} ({1})")
...
def calculate_metered_rate(
    ...
    suffix = f" ({days} days)" if factor < 1.0 else ""
    detailed_items.append({
        "description": _("Fixed Charge (Up to {0} m3)")
        .format(min_m3) + suffix,
        "amount": fixed_charge
    })
    ...
    if excess_m3 > 0:
        ...
        for tier in tiers:
            ...
            if m3_tier > 0:
                detailed_items.append({
                    "description": _("Excess: {0} m3 at {1}/m3")
                    .format(m3_tier, tier.price_per_m3),
                    "amount": cost
                })
                excess_amount += cost
    ...
    category_allows = frappe.db.get_value("Service Category",
                                          service_rate.service_category,
                                          "allow_discounts")
...
def calculate_adittion_fee(
    ...
    for fee in (service_rate.get("additional_fees") or []):
        ...
        detailed_items.append({"description": fee.description or
                               _("Additional Fee"), "amount": fee_amt})
...
def apply_discount_logic(
    ...
    # Logic for Juridical Persons remains (no discounts)
    if (
        getattr(subscriber, "subscriber_type", "Natural Person") ==
        "Juridical Person"
    ):
    ...
    # 1. Age-based rules logic
    if subscriber.birth_date:
        age = int(date_diff(reference_date, subscriber.birth_date) / 365.25)

        # Search for rules matching Age AND Cistern Requirement
        age_rules = frappe.get_all("Discount Rule", filters={
            "condition_type": "Age",
            "minimum_age": ["<=", age],
        ...
        # Filter by maximum_age manually to handle 0 as 'no limit'
        for r in age_rules:
            if r.maximum_age == 0 or age <= r.maximum_age:
                applicable_rules.append(r)

    # 2. Manual Benefit logic (if any)
    benefits = frappe.get_all("Benefit Discount", filters={
    ...
    # 3. Apply limit and totals
    applicable_rules.sort(key=lambda x: flt(x.fixed_amount) or
                          flt(x.discount_percentage), reverse=True)
    ...
    total_fixed = sum(flt(r.fixed_amount) for r in selected_rules
                      if r.discount_type == "Fixed Amount")
...
def finalize_breakdown(base_results, discount_pct, total_fixed, rules):
    ...
    # 1. Calculate Pct Discount
    pct_discount = flt(discountable * (discount_pct / 100))

    # 2. Total Discount (Pct + Fixed)
    total_discount_amount = pct_discount + flt(total_fixed)

    # Safety: Discount can't be more than the discountable sum
    ...
    if total_discount_amount > 0:
        ...
        items.append({
            "description": _("Applied Discounts ({0})").format(rule_list),
...
def generate_bills_and_link_advances(gap_list):
    ...
    for gap in gap_list:
        try:
            ...
            # 1. Calculate dates required for the bill's validation
            s_start = date(fiscal_year, m_num, start_day)
            s_end = add_days(add_months(s_start, 1), -1)

            # 2. Create and Submit the Bill
            # submit() will internally call apply_advance_payments()
            ...
            frappe.log_error(f"Gap Error {contract_id}: {str(e)}", "Billing")
