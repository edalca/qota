...
class DebtLedgerEntry(Document):
    ...
    if TYPE_CHECKING:
        ...
        entry_type: DF.Literal["Monthly Fee",
                               "Connection Fee",
                               "Late Fee",
                               "Reconnection Fee",
                               "Other Fee"]
    ...
    def on_update(self) -> None:
        """
        Triggers after saving. Synchronizes the status
    ...
    def apply_advance_payments(self) -> None:
        ...
        advance = frappe.db.sql("""
            ...
            WHERE parent.service_contract = %s
              ...
              AND item.balance > 0
            LIMIT 1
        """, (self.service_contract, self.billing_period), as_dict=True)
    ...
    def on_cancel(self):
        ...
        """
        frappe.throw(_(
            "Debt Ledger Entries cannot be cancelled manually. "
            "This creates accounting inconsistencies.")
        )


def clear_and_delete_debt(
    ...
    """
    debt_id: str | None = frappe.db.get_value("Debt Ledger Entry", {
    ...
    if flt(debt.paid_amount) > 0:
        formatted_paid = frappe.format_value(
            debt.paid_amount,
            {"fieldtype": "Currency"}
        )
        frappe.throw(_(
...
def make_debt_ledger_entry(
    ...
    if flt(amount) <= 0:
        frappe.throw(_(
            "Amount must be greater than zero to create a Debt Ledger Entry."
        ))
