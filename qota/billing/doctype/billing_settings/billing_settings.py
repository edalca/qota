...
class BillingSettings(Document):
	...
	if TYPE_CHECKING:
		...
		default_connection_fee: DF.Currency
	...
	def validate_cycle_day(self):
		"""Ensure the cycle start day is valid (1-28 to avoid issues with February)."""
		if self.cycle_start_day < 1 or self.cycle_start_day > 28:
			frappe.throw(
				_("Cycle Start Day must be between 1 and 28 to ensure compatibility with all months.")
			)

	def validate_percents(self):
		"""Validate that percentage values are within logical bounds."""
