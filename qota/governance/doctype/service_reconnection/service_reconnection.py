...
class ServiceReconnection(Document):
	...
	def enforce_fee_rules(self) -> None:
		...
		last_suspension = frappe.db.get_value(
			"Service Suspension",
			{"name": self.service_suspension, "status": "Executed", "docstatus": 1},
		...
		if last_suspension:
			is_free = (
				last_suspension.suspension_type == "By Request" or last_suspension.reason == "Maintenance"
			)

			if is_free:
				self.reconnection_fee = 0
				if not self.remarks:
					self.remarks = _("Free Reconnection: Previous suspension was due to {0}.").format(
						last_suspension.reason
					)
	...
	@frappe.whitelist()
	def execute_reconnection_logic(self, initial_reading: float, execution_date: str) -> None:
		"""
		Technically restores the water supply and reactivates the contract.
		"""
		if self.status not in ["Paid", "Scheduled"]:
			frappe.throw(_("Cannot execute. Payment is required for this reconnection."))
		...
		update_contract_property(
			...
			data={
				...
				"description": _("Service restored via technical order {0}").format(self.name),
			},
		)
		frappe.db.set_value("Service Contract", service_contract, "reactivation_date", self.execution_date)
	...
	def create_start_reading(self) -> None:
		...
		reading.remarks = _("Start reading (Reconnection Order {0})").format(self.name)
